...
const TeacherLogs: React.FC<TeacherLogsProps> = ({ teacherId }) => {
  const { logs, loadingLogs, hasMore, fetchLogsByUser, deleteLog, resetPagination } = useLogsStore();
  ...
  useEffect(() => {
    ...
    if (teacherId) {
      fetchLogsByUser(teacherId);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [teacherId]);

  // Deduplicate logs based on ID (server already filters by teacherId)
  ...
  const handleSearch = () => {
    resetPagination();
    fetchLogsByUser(teacherId, searchTerm, startDate, endDate, true);
  };

  // Handle load more
  const handleLoadMore = () => {
    fetchLogsByUser(teacherId, searchTerm, startDate, endDate, false);
  };
  ...
  const clearFilters = () => {
    ...
    setEndDate('');
    resetPagination();
    fetchLogsByUser(teacherId, '', '', '', true);
  };
    ...
    <div className="bg-base-100 rounded-none border border-base-300 p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold martian-mono text-primary">My Activity Logs</h2>
            <p className="text-xs text-base-content/60 font-normal italic">
              Track your teaching activities and actions
            </p>
          </div>
        </div>
      ...
      <div className="space-y-4">
        {loadingLogs && logs === null ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-sm martian-mono font-bold text-primary mb-2">No Activity Logs</h3>
        ...
        <div className="mt-6 p-4 bg-base-50 rounded-lg">
          <div className="text-center">
            <p className="text-sm text-base-content/60">
              Showing {uniqueLogs.length} log{uniqueLogs.length !== 1 ? 's' : ''} of your activities
            </p>
          </div>
        </div>
      )}
