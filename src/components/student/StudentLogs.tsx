...
const StudentLogs: React.FC<StudentLogsProps> = ({ studentId }) => {
  const { logs, loadingLogs, hasMore, fetchLogsByStudent, resetPagination } = useLogsStore();
  ...
  const handleSearch = () => {
    resetPagination();
    fetchLogsByStudent(studentId, searchTerm, startDate, endDate, true);
  };

  // Handle load more
  const handleLoadMore = () => {
    fetchLogsByStudent(studentId, searchTerm, startDate, endDate, false);
  };
  ...
  const clearFilters = () => {
    ...
    setEndDate('');
    resetPagination();
    fetchLogsByStudent(studentId, '', '', '', true);
  };
    ...
    <div className="bg-base-100 rounded-none border border-base-300 p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold martian-mono text-primary">Activity Logs</h2>
            <p className="text-xs text-base-content/60 font-normal italic">
              Track all activities and events related to your account
            </p>
          </div>
        </div>
